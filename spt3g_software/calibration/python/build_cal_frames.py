...
class BuildBoloPropertiesMap(object):
    ...
    def __init__(self, drop_original_frames=True, fiducial_detectors=[],
        ...
        *not* specified, five detectors near the middle of the focal plane present
        ...
        If `pointing_cut_threshold` a number (and not `None`), then an additional
        cut will be used to remove outlier RCW38 observations from the set that
        are used to define the pointing offsets. The function will find all
        bolometers in the same pixel as the bolometer of interest. The median
        offset is then computed over all RCW38 observations for all these
        bolometers. RCW38 observations for the bolometer of interest are cut if
        their offsets differ from the median reference offsets by more than
        `pointing_cut_threshold`.
        '''
        ...
        self.props = {}
        self.dfmux_tf = None
        self.bpm_nominal = None
        self.pointing_cut_threshold = pointing_cut_threshold
    ...
    def __call__(self, frame):
        if frame.type == core.G3FrameType.EndProcessing:
            ...
            xoffsets_all = numpy.array([self.bpm_nominal[bolo].x_offset \
                                        for bolo in self.bpm_nominal.keys()])
            yoffsets_all = numpy.array([self.bpm_nominal[bolo].y_offset \
                                        for bolo in self.bpm_nominal.keys()])
            bolonames = numpy.array([bolo for bolo in self.bpm_nominal.keys()])
            for bolo in self.props.keys():
                p = BolometerProperties()

                if self.pointing_cut_threshold is not None:
                    if 'xoffsets' in self.props[bolo] and bolo in self.bpm_nominal.keys():
                        bolos_samepixel = bolonames[(xoffsets_all==self.bpm_nominal[bolo].x_offset) & \
                                                    (yoffsets_all==self.bpm_nominal[bolo].y_offset)]

                        # Ideally we would have error bars on these measurements...
                        x_offset = numpy.array([self.props[bolo]['xoffsets'][i] - \
                                                numpy.mean([self.props[fd]['xoffsets'][i] for fd in self.fiducial_detectors]) \
                                                for i in range(len(self.props[bolo]['xoffsets']))])
                        y_offset = numpy.array([self.props[bolo]['yoffsets'][i] - \
                                                numpy.mean([self.props[fd]['yoffsets'][i] for fd in self.fiducial_detectors]) \
                                                for i in range(len(self.props[bolo]['xoffsets']))])

                        x_offset_others = []
                        y_offset_others = []
                        for b in bolos_samepixel:
                            if 'xoffsets' in self.props[b]:
                                x_offset_others += [self.props[b]['xoffsets'][i] - \
                                                    numpy.mean([self.props[fd]['xoffsets'][i] for fd in self.fiducial_detectors]) \
                                                    for i in range(len(self.props[b]['xoffsets']))]
                                y_offset_others += [self.props[b]['yoffsets'][i] - \
                                                    numpy.mean([self.props[fd]['yoffsets'][i] for fd in self.fiducial_detectors]) \
                                                    for i in range(len(self.props[b]['yoffsets']))]
                        x_offset_others = numpy.array(x_offset_others)
                        y_offset_others = numpy.array(y_offset_others)
                        xcenter = numpy.median(x_offset_others[numpy.isfinite(x_offset_others)])
                        ycenter = numpy.median(y_offset_others[numpy.isfinite(y_offset_others)])
                        offset_from_nominal = numpy.sqrt((x_offset - xcenter)**2. + \
                                                         (y_offset - ycenter)**2.)

                        x_offset_inrange = x_offset[numpy.isfinite(offset_from_nominal)]\
                                           [offset_from_nominal[numpy.isfinite(offset_from_nominal)]<self.pointing_cut_threshold]
                        y_offset_inrange = y_offset[numpy.isfinite(offset_from_nominal)]\
                                           [offset_from_nominal[numpy.isfinite(offset_from_nominal)]<self.pointing_cut_threshold]

                        if len(x_offset_inrange)>0:
                            p.x_offset = robust_avg(x_offset_inrange)
                            p.y_offset = robust_avg(y_offset_inrange)
                        else:
                            p.x_offset = numpy.nan
                            p.y_offset = numpy.nan
                else:
                    if 'xoffsets' in self.props[bolo] and bolo in self.bpm_nominal.keys():
                        p.x_offset = robust_avg([self.props[bolo]['xoffsets'][i] - \
                                                 numpy.mean([self.props[fd]['xoffsets'][i] for fd in self.fiducial_detectors]) \
                                                 for i in range(len(self.props[bolo]['xoffsets']))])
                        p.y_offset = robust_avg([self.props[bolo]['yoffsets'][i] - \
                                                 numpy.mean([self.props[fd]['yoffsets'][i] for fd in self.fiducial_detectors]) \
                                                 for i in range(len(self.props[bolo]['yoffsets']))])
