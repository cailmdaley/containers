...
# Do things?
run="python get_temperatures.py $LOCAL_PATH -o $OUTPUT"
echo "Doing $run"
$run
echo ""

# get datas back
echo "Sending $OUT home"
for file in `ls $OUT`; do
    echo "Decquiring $file"
    globus-url-copy -cd file://$OUT/$file gsiftp://gridftp.grid.uchicago.edu:2811/cephfs/spt/user/ndhuang/microphonics/$file
done
