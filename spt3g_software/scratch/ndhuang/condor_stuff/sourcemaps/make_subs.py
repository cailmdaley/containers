...
mem = '32G'
logname = 'sourcemaps'
exec = '/home/ndhuang/spt_code/spt3g_software/scratch/ndhuang/condor_stuff/sourcemaps/run_sourcemaps.sh'
outdir_base = '/spt/user/ndhuang/{}_perbolo_withpointing'.format(source)
extras = ''
...
for id in obsids:
    outdir = os.path.join(outdir_base, str(id))
    ...
    calfile = os.path.join('/spt/user/production/calibration/calframe', source, 
                           str(id) + '.g3')
    args = "'{calfile} {infile}' '{outfile}' '{obsid}'".format(calfile = calfile,
          infile = ' '.join(infiles), outfile = outdir, obsid = id)
    substr = sub.format(exec = exec,
                        args = args,
                        inputfiles = '/home/ndhuang/spt_code/spt3g_software/scratch/ndhuang/sourcemaps_perbolo_withpointing.py',
                        outputfiles = '', 
                        extras = extras,
                        outlog = outlog,
                        errorlog = errorlog,
