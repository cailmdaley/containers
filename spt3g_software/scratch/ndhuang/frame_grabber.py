...
class FrameGrabber(object):
    '''
    Grab contiguous sets of frames that meet `condition` (and possibly
    frames before and after).  This module adds a key
    'FRAMEGRABBER_SPLIT' to the first frame of each contiguous
    section.  If you use FrameGrabber.issplit to detect the splits,
    the key is deleted.
    ...
    '''
    def __init__(self, condition, extra_frames = 0, **condition_args):
    ...
    def issplit(fr):
        ...
        if split:
            fr.pop('FRAMEGRABBER_SPLIT', None)
        return split
