...
class OnlineVerifier:
    ...
    def __init__(self, obs, check_continuity=True, check_hk=True,
                 check_times = True, check_nans = True):
        """
        Arguments
        ---------
        obs : string
    ...
    def __call__(self, frame):
        ...
        (3) The times generated by core.G3Timestream match the recorded times
        (4) The pointing timestreams do not contain NaNs
        """
        ...
        if self.check_continuity:
            ...
            if self.last is not None:
                if tsmap.sample_rate == 0:
                    raise RuntimeError(
                        'Observation {} has a zero-rate scan frame at frame {} ({})'.format(
                            self.obs, self.count - 1, str(tsmap.start)))
        ...
        if self.check_times:
            tsmap = frame['RawTimestreams_I']
            dt = np.array([t1.time - t2.time for t1, t2 in
                           zip(tsmap.times(), frame['DetectorSampleTimes'])])
            dt = abs(dt) / core.G3Units.s
            if np.any(dt > 1e-6):
                raise RuntimeError(
                    'Observation {} has bad times in detector timestreams.'.format(self.obs))
            
        if self.check_nans:
            keys_to_check = ['OnlineBoresightRa', 'OnlineBoresightDec']
            for key in keys_to_check:
                if np.isnan(frame[key]).any():
                    raise RuntimeError(
                        'Observation {} has NaNs in {}'.format(self.obs, key))
