...
import glob
im = sorted(glob.glob('/poleanalysis/sptdaq/calresult/calibration/RCW38-pixelraster/maps/*'))[-40:]
import numpy as np
...
for sample_id in sorted(data.keys()):
    ...
    pipe.Add(lambda fr: fr.type == core.G3FrameType.Map) # Drop Cal Frames    
    pipe.Add(core.G3Writer, filename = '/poleanalysis/javva/rcw38_singlebolo_coadds/deep/%s_coadd.g3'%sample_id)
    pipe.Run()
