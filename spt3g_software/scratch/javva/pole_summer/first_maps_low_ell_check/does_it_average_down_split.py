...
for d in sorted(list(dts.keys())):
    ...
    if bun_num in [8,26,21,20]:
        continue
    g = core.G3Writer('/big_scratch/javva/maps_2019_lowell/individual_bundles_%s_strict_cuts_LR_bundle_%s.g3'%(rt,str(bun_num)))
    for k in coadd.keys():
...
import pickle
with open('/big_scratch/javva/maps_2019_lowell/split_%s_strict_cuts_defs_150.pkl'%rt, 'wb') as handle:
