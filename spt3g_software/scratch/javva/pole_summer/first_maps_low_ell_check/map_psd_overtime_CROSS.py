...
for enu, i in enumerate((all_mps)):
        ...
        for m1,m2 in [(0,3),(1,4),(2,5)]:
                ...
                ma = map_analysis.calculateCls(f[m1],cross_map = f[m2], apod_mask = numpy.asarray(apod), qu=True, ell_min=10, ell_max = 2500, delta_ell = 12)
                bands = ['90','150','220']
        ...
        clz['U'] = ucls
        clz['c'] = ma['ell']
        with open('CROSS_ps_summary_singles_%s_strict_cuts.pkl'%rt, 'wb') as handle:
