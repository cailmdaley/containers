...
job_root = 'sims_maps_filtered_synfast_p4'
...
script = '/home/javva/spt3g_software/scratch/javva/pf_k_space_maps/makemaps_poly4_npf.py'
...
#        print(jobname)
        nonsense = '/spt/user/nwhitehorn/sptpol/lowell/scripts/nonsense.g3'
        obs = '-15997020'
...
#        print(data)
        ...
        infiles_all = [nonsense] + [cal] + sorted(data) + [a]
#        print(infiles)
        infiles = [nonsense] + [cal] + sorted(data)
#        print(infiles)
        args_in1 = [os.path.basename(dat) for dat in infiles]
        args_in = ['./'+d for d in args_in1]

        extra_args= '-s'
        args = '{infiles} -s {extra_extra} -o {outfile}'.format(infiles = ' '.join(args_in), 
