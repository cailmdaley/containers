...
#    fr = list(core.G3File(sorted(lefts)[i]))[0]
    print('adding in '+ fle)
    fl_nm = sorted(lefts)[i]
#    if 'Id' not in fr or fr['Id'] != 'ra0hdec-57.5-150GHz':
#        continue
    num_files = num_files + 1
    fil_list = np.append(fil_list,fl_nm)
    defs_dict[bun_num] = fil_list
...
import pickle
with open('/spt/user/javva/lowell/maps/paired_lt/lead_trail_defs_dict_correct_bundle_defs.pkl', 'wb') as handle:
