...
for fle in g3_bundles['sorted'][90:100]:
    ...
    const = {}
    const['T'] = nc/np.mean(np.asarray(fr['Wpol'].TT))
    const['Q'] = nc/np.mean(np.asarray(fr['Wpol'].QQ)) 
    const['U'] = nc/np.mean(np.asarray(fr['Wpol'].UU))

    new_weights = core.G3SkyMapWeights()
    new_weights.weight_type = core.WeightType.Wpol
    new_weights.TT = (fr['Wpol'].TT/np.mean(np.asarray(fr['Wpol'].TT)))*nc
    new_weights.TU = (fr['Wpol'].TU/np.mean(np.asarray(fr['Wpol'].TU)))*nc
    new_weights.TQ = (fr['Wpol'].TQ/np.mean(np.asarray(fr['Wpol'].TQ)))*nc
    new_weights.UU = (fr['Wpol'].UU/np.mean(np.asarray(fr['Wpol'].UU)))*nc
    new_weights.QU = (fr['Wpol'].QU/np.mean(np.asarray(fr['Wpol'].QU)))*nc
...
for p in ('T', 'Q', 'U'):
    f[p] = coadd[p]
f['Wpol'] = coaddweights
core.G3Writer( '/big_scratch/javva/reweight_coadds/100_small_bundle_not_reweighted.g3')(f)
num_files = 0 
...
import pickle
with open('bundle_defs_100_small_map_coadd_not_reweighted.pkl', 'wb') as handle:
