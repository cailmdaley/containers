...
class RenormalizeTS(object):
    def __init__(self):
        self.pixlist = None
    def __call__(self, fr):
        global kcmbES, ints, pixlist
        if 'KcmbElnodSlopes' in fr:
            kcmbES = fr['KcmbElnodSlopes']
        ...
        if 'CalTimestreams' in fr:
            out = core.G3TimestreamMap()
            ints = fr['CalTimestreams']
            for pix in self.pixlist.values():
                if pix[0] not in ints:
                    try:
                        out[pix[1]] = ints[pix[1]]
#                        print('volcano')
                        continue
                    except:
                        continue
                if pix[1] not in ints:
                    try:
                        out[pix[0]]= ints[pix[0]]
                        continue
                    except:
                        continue
                if len(pix) != 2 or pix[0] not in ints or pix[1] not in ints:
                    continue
                renorm = kcmbES[pix[0]]/kcmbES[pix[1]]
#                print('non-volcano')
                out[pix[0]] = ints[pix[0]]*renorm
                out[pix[1]] = ints[pix[1]]
            fr['RenormCTS'] = out
pipe.Add(RenormalizeTS)
pipe.Add(core.Delete, keys=['CalTimestreams'])
pipe.Add(core.Rename, keys={'RenormCTS': 'CalTimestreams'})
