...
class ReplaceCalframePointing(object):
    def __init__(self, new_pointing_file):
        self.new_pointing_file = new_pointing_file
    def __call__(self, frame):
        if frame.type != core.G3FrameType.Calibration:
            return
        pointing_frame = core.G3File(self.new_pointing_file).next()
        changed_keys = set(pointing_frame['PointingOffsetX'].keys())
        all_keys = set(frame['BolometerProperties'].keys())
        keys = changed_keys & all_keys
        newboloprops = copy.deepcopy(frame['BolometerProperties'])
        for k in keys:
            newboloprops[k].x_offset = pointing_frame['PointingOffsetX'][k]
            newboloprops[k].y_offset = pointing_frame['PointingOffsetY'][k]
        frame['NewBoloProps'] = newboloprops
        core.Delete(frame, ['BolometerProperties'])
        core.Rename(frame, {'NewBoloProps': 'BolometerProperties'})
        return 

_ReplaceCalframePointing = ReplaceCalframePointing('/home/sguns/newbeams/new_pointing_old_mean.g3')
...
wafer_lst = ['w203', 'w201', 'w188', 'w174', 'w177', 'w176', 'w172', 'w180', 'w181']
...
pipe.Add(_ReplaceCalframePointing)
