...
pipe.Add(calibration.ApplyTCalibration, Input='TimestreamsWatts', Output='CalTimestreams', OpacityCorrection=False)
...
for band in [90, 150, 220]:
    pylab.hist(data[:,0][data[:,1] == band]/core.G3Units.uK, label=str(band), bins=numpy.linspace(0, 3500, 100))
pylab.legend()

pylab.xlabel('NET_CMB [uK-rt(s)]')
pylab.ylabel('# of bolos')

pylab.figure()
for band in [90, 150, 220]:
	n = data[:,0][data[:,1] == band]
	pylab.scatter(data[:,2][data[:,1] == band]/core.G3Units.MHz, n/core.G3Units.uK, marker='.', s=1, label=str(band))
	print('%d GHz Bolometer Average: %.1f uK-sqrt(s)' % (band, numpy.sum(n/(n**2))/numpy.sum(n**-2)/core.G3Units.uK))
	print('%d GHz Array Average: %.1f uK-sqrt(s)' % (band, numpy.sum(n/(n**2))/numpy.sum(n**-2)/core.G3Units.uK/len(n)**.5))

pylab.legend()
pylab.ylabel('NET_CMB [uK-rt(s)]')
