...
obsidstr = '71819554'
g3files = glob.glob('/spt/data/bolodata/fullrate/saturn-pixelraster/'+obsidstr+'/0*.g3')
g3files.sort()
...
#calfile = '/spt/user/production/calibration/calframe/saturn-pixelraster/'+obsidstr+'.g3'
calfile = '/spt/user/production/calibration/calframe/calibrator/71481722.g3'
...
el_saturn = 22.75*np.pi/180.
scanspeed_onsky = 0.3*np.cos(el_saturn)
arcmin_per_sample = 60.*scanspeed_onsky/152.6
nx = 300
xtemp = (np.arange(nx)-nx/2)*arcmin_per_sample

# step through files and scans, eyeball if there's a saturn crossing; if so, try to fit the leading and trailing halves to separate gaussians

## !!!
#goodnames = ['005.12.1.2.2548','005.12.1.2.2548']
## !!!
...
#                    print dtemp[imax]
#                    print sdtemp
                    if np.abs(dtemp[imax])/sdtemp > 100. and imax > nx/2 and imax < len(dtemp)-nx/2: # Saturn detected!
...
pickle.dump(outdict,open('/spt/user/tcrawfor/public/saturn_fit_out_16apr19.pkl','w'))
