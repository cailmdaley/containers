...
#################################
def get_cal_obs_before_field_scan(input_string,dir='/spt/data/bolodata/downsampled/calibrator/'):

    '''
    get closest calibrator stare before field obs. 
    input can be obsid or file name. output is obsid.
    '''

    if len(input_string) == 8:
        field_obsid = input_string
    else:
        tempstrs = input_string.split('/')
        if 'g3' in tempstrs[-1]:
            field_obsid = tempstrs[-2]
        else:
            field_obsid = tempstrs[-1]

    field_obsid_int = np.int(field_obsid)
    calfiles_all = glob.glob(dir+'*/0000.g3')
    calfiles_all.sort()
    cal_obsids_all_int = np.asarray([np.int(cf.split('/')[-2]) for cf in calfiles_all])
    dt = cal_obsids_all_int-field_obsid_int
    whneg = np.where(dt < 0)[0]
    index = whneg[np.argmin(np.abs(dt[whneg]))]
    cal_obsid = str(cal_obsids_all_int[index])
