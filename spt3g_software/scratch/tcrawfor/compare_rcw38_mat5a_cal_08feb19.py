...
#
#rcw38files = glob.glob(cfdir + 'RCW38-pixelraster/6???????.g3')
#rcw38files.sort()
#rcw38files2 = glob.glob(cfdir + 'RCW38/6???????.g3')
#rcw38files2.sort()
#for rfile in rcw38files:
#    obsid = rfile.split('/')[-1].split('.')[0] 
#    if np.int(obsid) < 64000000:
#        continue
...
#            pass
#
#mat5afiles = glob.glob(cfdir + 'MAT5A-pixelraster/6???????.g3')
#mat5afiles.sort()
#mat5afiles2 = glob.glob(cfdir + 'MAT5A/6???????.g3')
#mat5afiles2.sort()
...
#            pass
#
#pickle.dump(outd,open('/spt/user/tcrawfor/public/cals_rcw38_mat5a_08mar19.pkl','w'))
