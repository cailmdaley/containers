...
def alteralm(alm_in, nside_in, nlmax=None, fwhm_in=None, fwhm_out=None,
    ...
    # process parameters
    if isinstance(alm_in, basestring):
    ...
    if beam_in is not None:
        if isinstance(beam_in, basestring):
    ...
    if beam_out is not None:
        if isinstance(beam_out, basestring):
...
def write_map(filename, m, nest=False, dtype=np.float64, fits_IDL=True,
    ...
    if column_units is None or isinstance(column_units, basestring):
...
def read_map(filename, field=0, dtype=np.float64, nest=False, partial=False,
    ...
    if partial:
        ...
        field = tuple(f if isinstance(f, basestring) else f+1
                      for f in field)
    ...
    if return_names:
        rnames = [f if isinstance(f, basestring) else names[f] for f in field]
    else:
        rnames = None
...
def hotspot(m, return_values=False, extrema_outfile=None,
    ...
    maxima_outfile : string, optional
    minima_outfile : string, optional
    ...
    minval : 1D array, optional
    maxval : 1D array, optional
